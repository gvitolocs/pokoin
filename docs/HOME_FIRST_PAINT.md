...
| Guest local, signed-in merge | Shopify / Amazon recently viewed: persist **ids** locally, compact product rows for instant paint, account history on the server after login. | `localStorage pokoin.recentCardIds.{game}` plus **compact** `pokoin.recentCardTiles.{game}` (max 24, not desk JSON). Home paints those tiles on the first frame. `GET/PUT/POST /api/marketplace-recents?game=` on the writer merges **after** `ready` — one history per TCG. |
