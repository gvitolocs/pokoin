...
| Guest local, signed-in merge | Shopify / Amazon recently viewed: persist **ids** locally, compact product rows for instant paint, account history on the server after login. | `localStorage pokoin.recentCardIds` plus **compact** `pokoin.recentCardTiles` (max 24, not desk JSON). Home paints those tiles on the first frame. `GET/PUT /api/marketplace-recents` on 15T merges **after** `ready`. |
...
| `market/src/recents.js` | Local 24 ids. Signed-in list is `marketplace_user_recents` via `/api/marketplace-recents` (15T writer). Compact rail tiles (name, PKN, `_homepage.webp` URLs) live in `localStorage pokoin.recentCardTiles` so a new tab still first-paints Recently seen. **Do not** dump desk `card-page` JSON there. Desk identity is `pokoin.cardPage.v1.` (`card-page-cache.js`). `syncRemoteRecentCardIds` is the account merge — do not call it on LCP. |
