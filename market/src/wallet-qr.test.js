...
test('site receive codes carry the username and optional whole-PKN amount', () => {
  assert.equal(buildReceiveQr({ kind: 'site', username: 'ash' }), 'pokoin:u/ash');
  assert.equal(buildReceiveQr({ kind: 'site', username: 'ash', amount: '25' }), 'pokoin:u/ash?a=25');
...
test('parse site codes back into recipient + amount', () => {
  assert.deepEqual(parseScannedQr('pokoin:u/ash'), { kind: 'site', recipient: 'ash', amountPkn: '' });
  assert.deepEqual(parseScannedQr('pokoin:u/ash?a=25'), { kind: 'site', recipient: 'ash', amountPkn: '25' });
  assert.deepEqual(parseScannedQr('pokoin:u/ash?a=007'), { kind: 'site', recipient: 'ash', amountPkn: '7' });
});
