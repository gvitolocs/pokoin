-- 12 Sep inferred_sale is the second complete-book persist, not that day's
-- market. 11 Sep (051) archived the Pi clone. 12 Sep re-inserted the live
-- book (first_seen clustered that afternoon across Chaos Rising / Pitch
-- Black / Crown Zenith / Scarlet & Violet / …) and archiveMissing tagged
-- leftover cheap-25 product ids as inferred_sale at full stack qty.
-- Litleo Chaos Rising 014/086 (public 779910) plotted 175 copies / 58 PKN
-- and +163.6% from that lump; Best sellers ranked Chaos Rising commons by
-- those sample counts. Keep 12 Sep quantity_decreased.
-- SPA cache pokoin.cardSales.v7.
...
begin
  ...
  where obs.source = 'cardtrader_removed_sale'
    ...
    and not (
      ...
      and (obs.observed_at at time zone 'utc')::date in (date '2026-09-11', date '2026-09-12')
    )
    and obs.price_pkn > 0
