-- Listed-median rows were rolled from Pi snapshots on 2026-09-06 20:23 UTC
-- into marketplace_price_observations as source=cardtrader_removed_sale,
-- language EN, qty 1. They are asks, not sales. They became every
-- cardtrader_sold_daily row for 6 Sep (60,098 EN points) and polluted
-- last-observed / source_counts on marketplace_blueprint_price_*.
-- Never treat source_item_id listed-median-day:* as a sold comp.
...
where source = 'cardtrader_removed_sale'
  and (
    source_item_id like 'listed-median-day:%'
    or coalesce(metadata->>'observationKind', '') = 'listed_median_from_snapshot'
  );
