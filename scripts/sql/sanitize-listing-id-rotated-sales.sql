-- CardTrader marketplace product ids rotate when a seller edits a live
-- stack (price tweak). Pokoin then archives the old id as inferred_sale
-- with the full quantity, while the same seller still lists the same
-- qty/condition/language/blueprint under a new id.
-- Mewtwo Reverse Holo Promo (blueprint 206584 / public 413168): Meta TCG
-- qty 28 NM EN, nine days, nine listing ids, still live as 451350835.
-- Same listing_id filters (047 flicker, 052 still-listed) miss this.
...
join public.cardtrader_market_listing_snapshots live
  on live.provider = 'cardtrader'
 and live.blueprint_id is not distinct from h.blueprint_id
 and live.seller_account_id <> ''
 and live.seller_account_id = h.seller_account_id
 and live.quantity = h.quantity
 and live.condition is not distinct from h.condition
 and live.language is not distinct from h.language
 and public.cardtrader_listing_is_reverse(live.properties, false, '')
     is not distinct from public.cardtrader_listing_is_reverse(h.properties, false, '')
 and public.cardtrader_listing_is_first_edition(live.properties, false)
     is not distinct from public.cardtrader_listing_is_first_edition(h.properties, false)
where o.source = 'cardtrader_removed_sale'
  ...
  and h.provider = 'cardtrader'
  and h.archive_reason = 'inferred_sale'
  and h.external_listing_id = split_part(o.source_item_id, ':', 2)
  and h.removed_day = split_part(o.source_item_id, ':', 3)::date
  and (o.observed_at at time zone 'utc')::date is distinct from date '2026-09-11';
...
set
  ...
  archive_metadata = coalesce(h.archive_metadata, '{}'::jsonb) || jsonb_build_object(
    ...
    'reclassifiedBecause', 'same_seller_stack_still_listed'
  )
